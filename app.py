...
# Consolidated script for file integrity monitoring with Flask dashboard, adapted for Google Colab and local runs
...
# --- Config Section ---
MONITOR_DIR = "C:/Users/Arsal/Downloads/Crypto_File_Integrity/Crypto_File_Integrity/monitored_files"  # Directory to monitor
...
def init_db():
    ...
    with sqlite3.connect(DB_FILE) as conn:
        cursor = conn.cursor()
        # File hashes table (from original project)
        cursor.execute('''
        ...
        ''')
        # Merkle root table (from original project)
        cursor.execute('''
        ...
        ''')
        # Tamper log table (from original project)
        cursor.execute('''
        ...
        ''')
        # File history table (from dashboard)
        cursor.execute('''
...
async function loadFiles() {
  ...
  table.innerHTML = files.map(file => `
    <tr>
      ...
      <td class="p-2"><button onclick="viewHistory('${file.id.replace(/'/g, '\\'')}')"
         class="text-blue-600 hover:underline">View History</button></td>
    </tr>
  `).join('');
  ...
  tamperTable.innerHTML = tamperLog.map(log => `
    <tr>
      ...
      <td class="p-2">${log.new_root.slice(0, 10)}...</td>
...
async function viewHistory(fileId) {
  ...
  alert("File History:\n" + history.map(h =>
    `${new Date(h.timestamp).toLocaleString()} — ${h.change_type} (${h.algorithm.toUpperCase()}): ${h.hash.slice(0, 10)}...`
  ).join("\n"));
}
...
if __name__ == "__main__":
    ...
    with open(os.path.join(MONITOR_DIR, "test.txt"), "w") as f:
